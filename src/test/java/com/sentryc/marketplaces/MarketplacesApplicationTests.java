...
class MarketplacesApplicationTests {
    ...
    void getAllSellerData() {
        ...
        // When
        List<PagebleResponse> result = service.getAllSellerData();

        // Then
        assertNotNull(result);
        assertEquals(1, result.size());

        PagebleResponse response = result.get(0);
        ...
        assertNotNull(response.getProducerSellerInfos());
        assertEquals(1, response.getProducerSellerInfos().size());

        ProducerSellerInfo producerSellerInfo = response.getProducerSellerInfos().get(0);
        assertEquals(producerId.toString(), producerSellerInfo.getProducerId());
        assertEquals("ExampleProducer", producerSellerInfo.getProducerName());
        assertEquals("REGULAR", producerSellerInfo.getState().toString());
        assertEquals(sellerId.toString(), producerSellerInfo.getSellerId());
    }
}
